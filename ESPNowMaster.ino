...
//Canal usado para conexão, lembrando que temos 14 canais
#define CHANNEL 1

//Pinos que iremos ler (digitalRead) e enviar para os Slaves(Escravos). Podemos colocar mais, só preencher no array
//É importante que o código fonte dos Slaves tenha este mesmo array com os mesmos gpios na mesma ordem
uint8_t gpios[] = {23, 2};
...
//Exemplo:{0x24, 0x0A, 0xC4, 0x0E, 0x3F, 0xD1}, {0x24, 0x0A, 0xC4, 0x0E, 0x4E, 0xC3} "separe por virgula"
uint8_t macSlaves[][6] = {
...
void setup() {
  ...
  //Colocamos o ESP em modo station, todos são station, até o master
  WiFi.mode(WIFI_STA);
  ...
  //Para cada pino que está no array gpios, colocando todos os pinos que escolhemos no modo de leitura
...
//Função que irá fazer a leitura dos pinos que estão o array gpios e enviar os valores lidos para os outros ESPs
void send(){
...
//Função que serve de callback para nos avisar sobre a situação do envio que fizemos
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status) {
  char macStr[18];
  //Copiamos o Mac Address destino para uma string, ou seja, faz a conversão binário para texto
...
//Não precisamos fazer nada no loop pois sempre que recebemos o feedback do envio através da função OnDataSent
//nós enviamos os dados novamente, fazendo com que os dados estejam sempre sendo enviados em sequência
void loop() {
}
